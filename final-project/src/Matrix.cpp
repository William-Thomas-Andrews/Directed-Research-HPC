...
Matrix::Matrix(int num_rows, int num_columns) : rows(num_rows), columns(num_columns), size(num_rows*num_columns) { 
    matrix_array = (float *) malloc(size * sizeof(float));
    for (int i = 0; i < size; i++) { matrix_array[i] = 0; }
}

// Uniform dist [lower_bound, upper_bound]
Matrix::Matrix(int num_rows, int num_columns, float lower_bound, float upper_bound) : rows(num_rows), columns(num_columns), size(num_rows*num_columns) { 
...
Matrix::Matrix(float item, int num_rows, int num_columns) : rows(num_rows), columns(num_columns), size(num_rows*num_columns) { 
    matrix_array = (float *) malloc(size * sizeof(float));
    for (int i = 0; i < size; i++) { matrix_array[i] = item; }
}

// Vector input for floats
Matrix::Matrix::Matrix(const std::vector<float>& data, int num_rows, int num_columns) : rows(num_rows), columns(num_columns), size(num_rows*num_columns) { 
...
// Standard float array input input
Matrix::Matrix(float* data, int data_size, int num_rows, int num_columns) : rows(num_rows), columns(num_columns), size(num_rows*num_columns) { // C-style array input
...
Matrix& Matrix::operator=(const Matrix& other) { // Copy assignment
    ...
    rows = other.rows;
    columns = other.columns;
    matrix_array = (float*) malloc(other.size * sizeof(float));
...
std::vector<float> Matrix::get_data() const {
...
float Matrix::get_element(int row_index, int col_index) {
    if (rows <= row_index) {
...
float& Matrix::operator()(int row_index, int col_index) const {
...
Matrix operator+(float val, const Matrix& M) {
    Matrix return_matrix = Matrix(M.matrix_array, M.size, M.rows, M.columns);
    for (int i = 0; i < return_matrix.rows; i++) {
        for (int j = 0; j < return_matrix.columns; j++) {
            return_matrix(i, j) += val;
