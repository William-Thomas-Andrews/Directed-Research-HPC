...
The most interesting matrix operation I implemented was a simple matrix multiplication that takes in the regular matrix A as the left operand, and takes in a transposed B as the right operand. It accounts for the B operand being transposed and updates the indices accordingly. The goal of this new matrix operation is to promote cache locality. How does it do this? Well, for example, when performing the reguler 'i-j-k' matrix multiplication operation, we start examining all of the rows of A (which is 'i'), and due to my Matrix class being row-major (meaning the contiguous 1-dimensional data array is ordered by rows) each row of A is next to eachother in memory, which is helpful for cache prediction in using cache locality. However, this process is a double-edged sword because when we examine the columns of B (which is 'j') and then going further into the innermost loop of rows of B (which is 'k') we jump by 'B_cols' amount every 'k' because our access will look like this for the contiguous data array for B: "B->data_array[k * B_cols + j]", which means none of the data is local (contiguous) to eachother, and we may receive a lot of cache misses. Like I said, to fix this I implemented a version that reorients the data of B such that it accesses B like so: "B->data_array[j * A_cols + k]", where the varying 'k' is incremented one by one and does not lead to any jumps with multiplication. The accesses to A in this variation remain unchanged: "A->data_array[i * A_cols + k]". This means that we are now using data in the last loop that consists of small "vectors" of B_rows amount of elements. This improves cache locality and speeds our matrix operation up to be the fastest one we have. Even with the cost of transposition, that cost is small compared to the efficiency of the loss of many cache misses in the new algorithm, so the transposition cost is negligable and the new algorithm is still much faster than the rest.
